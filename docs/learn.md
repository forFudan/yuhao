...
{: .note}
在进行拆分前，首先需要「确认」某个字根到底存不存在，是否合规，也就是检查字根的「内在属性」。比如「土」「士」，两横的长度直接决定了字根的异同，再比如「王」中间如果被笔画「穿心」，则不再是「王」。核心规则有三条：散件不分割、竖向不包夹、横间不穿心。这部分的讨论作为进阶内容，故而于此处跳过。若有兴趣，可以在后续章节阅读。
...
<!-- - 「罒」中如果包含完整的笔画，应该拆如「囗〢」。例如：「囬」中有「二」，故而应该拆成「囗〢二」。 -->

<!-- ### 易混淆字根辨析

以下列举出部分形近字根，讨论它们在宇浩拆分中被分离或者被合并的原因。 -->
