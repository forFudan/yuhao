...
- 每个字都可能有若干**兼容拆法**，目的是兼容台湾、香港、大陆古籍的字形标准。比如「起=走己」`DBjk` （大陆标准）和「起=走巳」`DCKk` （台湾标准）兼收。

宇浩输入法的笔顺选取，依照大陆标准《GF 3003-1999 GB13000.1字符集汉字字序（笔画序）规范》。同时，也兼容台湾标准的笔顺。比如「變=言糹糹夂」`UBBE` （大陆标准）和「變=糹言糹夂」`BUBE` （台湾标准）兼收。
