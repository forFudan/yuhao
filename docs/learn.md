...
这里对比一下五笔、郑码、宇浩、虎码等十余种常见输入法在不同汉字字符集下[重码数据](./articles/statistics)，供大家参考。
...
- 如果拼音中有`Z`或`X`，那么可以选`K`来代替。
- 如果拼音中有`U`，那么可以选`V`来代替。
...
有些字根的使用频率极高，我们把它们的小码统一设置为`v`（原因是拼音不用v字母，所以这个字母比较闲置）。
...
如果要**单独**输入「被代表根」，只要在大小码后加`ff`即可。这类汉字并不多。
...
>- `户Ih`、`戶Ih`被归为一个字根，由「户」统摄，所以输入「戶」时，编码为`Ihff`。
...
答案是，如果方框里有**完整**的笔画，用「囗」`Nw`。否则一律用「口」`Lv`。
