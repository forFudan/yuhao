...
>竖：  
>J 早日山曰田，  
>K 甲申由禺电，下框(冂)敲右边(攴)，重門兼繁鹵，虫贝水上见。  
>L 口是心非  
...
>1. 字根取大
>
>以上优先级，还可以高度归纳为：**少>顺>散>连>交>断>理>大**。
...
>- 「算」字拆成〔竹目卄〕，只有三根，所以我们取全部根，也就是〔𥫗Qv 目Mv 廾So〕的大码，即 QMS。此时，注意到不足四码，故而补上最末根的小码 o。输入 QMSo 即可出字。
>- 「织」字拆成〔纟口八〕，只有三根，所以我们取全部根大码，也就是 VLT。此时，注意到不足四码，故而补上最末根的小码 b。输入 VLTb 即可出字。
...
>附属字根举例：
>
>- 「羽」、「习」被归为一个字根，由「羽」统摄。所以「羽」的编码为 `Xuu`，「习」字的编码为 `xuuu`。
...
>- 「事情」：「事」的编码为`〔{于下}口彐・ALXv〕`，「情」的编码为`〔心龶月・YHWy〕`。因此，「事情」的编码为`〔{于下}口心龶・ALYH〕`。
