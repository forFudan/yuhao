...
[使用字根练习页面来练习最常用的100个字根](https://zhuyuhao.com/yuhao/v2/practice/practice_100.html)。
...
>1. 字根取大
>
>以上优先级，还可以高度归纳为：**少>顺>整>散>连>交>断>大**。
...
结构完整，指的是一个拆分，不能过份破坏汉字的整体性。故而「囗」「勹」「冂」等包围结构不拆散。
...
注意到，「宇浩」的部分字根中，有撇变竖的附属根，如：「千」字根的「丨」变成「丿」，「干」字根的「丨」变成「丿」。

在多种拆分同时存在的情况下，我们避免这类字根出现。

{: .example }
>比如：
>
>- 「失」拆「丿夫」而不拆「牛乀」，因为「牛」的一竖是撇。
>- 「井」拆「二{介下}」而不拆「キ丨」，因为因为「キ」的一竖是撇。
...
>- 「好」字拆成〔女子〕，只有两根，所以我们取全部根，也就是〔女Cn 子Vk〕的大码，即 CV。此时，注意到不足四码，故而补上最末根的小码 k。注意到仍然不足四码，于是再添上首根的小码 a。输入 CVkn 即可出字。
...
「旬」的外框「勹」，同「敖」左下的「𠂊」都是撇加横折钩。宇浩输入法不做区分，都视为「月」的附属根`Wy`。
