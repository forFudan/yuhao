...
††：两版五笔都不使用 Z 键，因为它比较难按。Z 键空出后，还能作为反查引导、通配符使用。宇浩继承了这个特点，不使用 Z 键，因此所有五笔输入法都可以直接挂载宇浩输入法的码表，并实现 Z 键的完整功能。有优点就有缺点。不使用 Z 键，也会造成编码空间的下降。在全码4码的情况下，宇浩的编码空间只有其他26键输入法的85.5%。这意味着对宇浩输入法的字根离散要求更苛刻，使之不适合超大词库。
