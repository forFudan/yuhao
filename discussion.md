...
- 打字派和檢字派的區别。檢字派，目標是一字一碼，儘可能在大字集上低重，注重大字集靜態重碼；打字派，目標是在常用字集上低重，將字的權重納考量，注重常用字集動態重碼。
- 字根派和規則派的區别。字根派，目標是通過字根的分佈，有時候加以隨機，來減少重碼。規則派，目標是通過規則的複雜度來減少重碼。

在使用形碼創作的過程中，我發現我是一個鍵盤派，一個打字派，一個字根派。我希望同筆畫的字根在鍵盤上集中分佈。我還希望在常用繁簡漢字（10000個左右）上減少選字的頻率，而在大字集上不追求極端低重。我更希望規則上不要太複雜，防止思路被打斷，比如判斷主副根，判斷取不取次未根，字的結構是上下還是左右等。
...
| ------------------------------ | ----------------------- | -------- | -------- | ------ | -------- | ------ |
| 簡體靜態重碼重碼數量           | 徐碼=**宇浩**           |          | 五筆98   | 虎碼   | 五筆86   | 鄭碼   |
| 簡體動態重碼重碼數量           | 虎碼=**宇浩**           |          | 徐碼     | 虎碼   | 五筆86   | 五筆98 | 鄭碼 |
| 常用字簡繁通打靜態重碼重碼數量 | 徐碼                    | **宇浩** | 虎碼     | 鄭碼   | 五筆98   | 五筆86 |
| 大字集重碼數量                 | 徐碼                    | **宇浩** | 五筆98   | 五筆86 | 鄭碼     | 虎碼   |
| 字根編碼規律性（檢字派）       | 鄭碼                    | 徐碼     | 五筆     |        | **宇浩** | 虎碼   |
| 鍵盤區位聚合度（打字派）       | 五筆                    |          | **宇浩** | 鄭碼   | 徐碼     | 虎碼   |
...
本着實用主義和現實主義，我將漢字的輸入分爲打字和檢字兩部分：

- 打字，指的是對於常用字的輸入。這要求確定性，保證在最常見8000個繁簡漢字混合狀態下，大多數碼位不存在重碼字，可以進行盲打。如果有重碼，也可以通過簡碼規避。
- 檢字，指的是生僻字輸入，要求輸入方案提供包括CJK的全部漢字。這部分的輸入不需要過分追求低重碼，因爲對於一個生僻字，就算輸入法低重，你也肯定要看一下屏幕進行確認。當然，不能翻頁太多，儘量保證在 GBK 字集下單個碼位的重碼數量不超過5個。
...
| 形碼方案 | GB2312 | 國字 | 常用繁簡 | GBK  | GB2312選重率 |
| -------- | ------ | ---- | -------- | ---- | ------------ |
| 徐碼     | 318    | 127  | 488      | 2902 | 0.11%        |
| 宇浩     | 318    | 216  | 916      | 4991 | 0.05%        |
| 五筆98   | 515    | 329  | 1679     | 6368 | 0.38%        |
| 虎碼     | 532    | 238  | 2076     | 7687 | 0.05%        |
| 五筆86   | 537    | 357  | 1741     | 6582 | 0.34%        |
| 鄭碼     | 563    | 311  | 1809     | 6590 | 0.59%        |
...
- 字根取一、二、三、未，優於取一、二、次未、未。這是因爲倒數第二根需要進行一些判斷，不够直觀。
...
- 分主副根，不如不分。因爲主副根需要進行一次判斷，以決定到底第三個根取「次未」還是「未」。
- 無結構碼，比有結構碼好。因爲結構碼需要判斷字的構架。
- 首根小碼不後置，比後置好。因爲後置首根小碼，會擠佔一個主根的大碼或未根的小碼，如果判斷首根失誤，會造成回改。
...
某些字形在常用字集内没有區分，或者共用 Uniocde 碼位。真正的字形區分往往在 CJK 拓展區，屬於檢字範疇。爲了不增加判斷難度，因此：

- 冫和ㄑ都拆成二
- 日曰除卻「曰汩㫚旮」外都一併爲「日」
- 木朩𣎳字源不同，但已經混同。术朮亦同，不作區分。
