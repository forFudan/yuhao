...
朱宇浩 创作 | 点击加入官方QQ群: [735728797](https://jq.qq.com/?_wv=1027&k=2OYDP4Tk)
...
这里对比一下五笔、郑码、宇浩、虎码等笔者比较喜欢的输入法，在不同汉字字符集下的单字全码的重码数量。[点击此处查看更多输入法的重码数据](https://zhuyuhao.com/yuhao/statistics.md)。其中：
...
| 宇浩                             | **304** |    **262** |      204 |      565 |     4937 | **0.05%**  | **0.15%**  | **0.17%**          |
| **五码定长 繁简通打**            |
...
[在开始之前，你可以下载挂载于小小输入平台的宇浩输入法](https://zhuyuhao.com/yuhao/assets/宇浩输入法-小小输入平台.exe)。打开 exe 文件后可以直接使用宇浩输入法打字。
...
在接下来的学习中，请[使用字根练习页面来练习最常用的50个字根](https://zhuyuhao.com/yuhao/practice_50.html)。然后可以[继续练习最常用的100个字根](https://zhuyuhao.com/yuhao/practice_100.html)。
...
笔划断开，指的是将字中的一个笔划断开，这是允许的。比如：

- 「我」拆成「丿扌戈」三个字根，其中的「扌」和「戈」两根的横本是一笔写成。但如果不断开这一横，就要拆成「丿扌乚丿丶」五个字根。为了让字根数量最少，我们将横断开，拆成「丿扌戈」三个字根。

笔划断开的拆分，往往是为了减少字根而不得已的操作，它的优先级较低。例如：

- 「果」拆作「日木」，不拆成「田木」，这是因为后者将中间的竖断开了。因为「字根相交」的优先级高于「笔画断开」，「果」应当拆成「日木」。

### 3.7. 拆分美观

拆分美观，或者说拆分直观，指的是一个拆分，一个人一看就应该这样拆。「直观」是个比较主观的标准，为了避免规则的不确定性，它的等级很低，并且还有以下的具体规则。
...
字根取大，指的是按照笔顺拆字时，让靠前的字根的笔划数尽量地多。「取大原则」，按定义，是一个兜底原则。它保证了最终只有一个候选方案能够胜出。比如：
...
从A到Y排列，一级简码字分别是`把那好不的一大地没是上中小回为这我得在个着了然出心`。在字根图中，一简字就是每个字母方框的左上角的汉字。
...
因此，我基于**实用主义**原则，对这两个字根不多加以区分，而采取更简单的方法。即：除了上述三组对立情况外，全部取「日」根。换句话说，「曰」根只需要在对立情况下才会使用，包括：「曰」「汩」「㫚」「旮」。
