...
- 最常用的1500字，全码下 0 重码。
- 最常用的3000字，全码下有 12 组重码，出简不出全为 0 组重码。
- 最常用的4500字，全码下有 53 组重码，出简不出全为 10 组重码。
...
†：例如在五笔中，「革」「鱼」，甚至「衤」「犭」等字根都被拆成了两个字根。但在其他输入法中，字根被完整保留。还宇浩中，还有不少完整的康熙部首，如「風」「僉」等。

††：两版五笔都不使用 Z 键，因为它比较难按。Z 键空出后，还能作为反差引导、通配符使用。宇浩继承了这个特点，不使用 Z 键，因此所有五笔输入法都可以直接挂载宇浩输入法的码表，并实现 Z 键的完整功能。我上面说过，有优点就有缺点。不使用 Z 键，也会造成编码空间的下降。在全码4码的情况下，宇浩的编码空间只有其他26键输入法的85.5%。这意味着对宇浩输入法的字根离散要求更苛刻，使宇浩不適合超大詞庫。
...
为了输入快捷，某些常用字我们只要打它的第一个字母，加一个空格就可以了，我们叫这些常用字为一级简码字。从A到Y排列，一级简码字分别是：`把那好不的一大地没是上中小回为这我得有个着了然以心`。
