...
这里对比一下五笔、郑码、徐码、虎码在不同汉字字符集下的单字全码的重码数量，以供参考。这些都是我比较喜欢、有亮点的输入法。仓颉虽然是五码定长，但它在繁体输入上地位很高，所以也将它纳入比较。加粗的数字代表本行最优值（重码数量在10个以内、静态重码在0.01%以内都算作同一水平）。排名按发明时间顺序：
...
|      | T    | 片                 | 卯({乐上}) a  八({介下}) b  人 e  彡({两撇}{癶右}) i  钅 j  自 k  用 o  手(龵) u    |
